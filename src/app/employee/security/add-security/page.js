...
export default function AddSecurityPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/employee/security/add-security', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to add security employee');
      }

      alert('Security employee added successfully!');
      setFormData({
        firstName: '',
        lastName: '',
        sex: '',
  ...
  return (
    <div className={styles.formContainer}>
      <Card title="Add Security Employee">
