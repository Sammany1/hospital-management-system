// filepath: /Users/alsammany/Documents/Personal/SPRING 25/Database/Project/GUI/hospital/src/app/employee/housekeeping/add-housekeeping/page.js
'use client';
...
export default function AddHousekeepingPage() {
  ...
  const validateField = (name, value) => {
    ...
    // area_assigned is optional for housekeeping_employee (can be null in employee_area_assigned or not present)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/employee/housekeeping/add-housekeeping', {
        method: 'POST',
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to add housekeeping employee');
      }
      
      alert('Housekeeping employee added successfully!');
      ...
      setFormErrors({});
      router.push('/employee/housekeeping'); 
    } catch (error) {
  ...
  return (
    <div className={styles.formContainer}> 
      <Card title="Add New Housekeeping Employee">
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <Button type="submit" variant="primary" disabled={isFormInvalid}>
            {isSubmitting ? 'Adding Employee...' : 'Add Housekeeping Employee'}
          </Button>
        </form>
      </Card>
    </div>
