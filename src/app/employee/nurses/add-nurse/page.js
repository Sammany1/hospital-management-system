...
import { Input, Button, Card } from '@/components/ui'; // Assuming components are in @/components/ui
...
export default function AddNursePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Correct API endpoint for adding a nurse
      const response = await fetch('/api/employee/add-nurse', { 
        method: 'POST',
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to add nurse');
      }
      ...
      setFormErrors({});
      router.push('/employee/nurses/view'); 
    } catch (error) {
      ...
      <Card title="Add New Nurse">
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.formField}>
            ...
            <Input name="state_or_province" id="state_or_province" value={formData.state_or_province} onChange={handleChange} placeholder="Enter state or province" />
