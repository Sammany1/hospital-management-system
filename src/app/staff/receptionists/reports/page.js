  ...
  useEffect(() => {
    async function fetchReportData() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch ${staffType.toLowerCase()} reports: ${response.status} ${response.statusText}`);
        }
    ...
    <div className={styles.container}>
      <Card title={`${staffType} Reports`}>
        ...
        {reportData && (
          <div className={styles.reportContent}>
            <div className={styles.reportSection}>
              ...
              <p className={styles.statItem}><span className={styles.statLabel}>Total {staffType}:</span> {reportData.totalReceptionists || reportData.totalStaff || 'N/A'}</p>
                ...
                <ul className={styles.statList}>
                  {Object.entries(reportData.shiftCoverage).map(([shift, count]) => (
                    <li key={shift} className={styles.statListItem}>
                      <span className={styles.statLabel}>{shift}:</span> {count} {staffType.toLowerCase()}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        {!loading && !error && !reportData && (
            <p>No report data available for {staffType.toLowerCase()} at this time.</p>
        )}
      </Card>
    </div>
  );
