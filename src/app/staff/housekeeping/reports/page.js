...
export default function HousekeepingReportsPage() {
  ...
  const staffType = "Housekeeping"; // Used for titles and messages
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const errorData = await response.json().catch(() => ({ 
            message: `Failed to fetch ${staffType.toLowerCase()} reports: ${response.status} ${response.statusText}` 
          }));
          throw new Error(errorData.message || `Failed to fetch ${staffType.toLowerCase()} reports: ${response.status} ${response.statusText}`);
        }
        const data = await response.json();
        setReportData(data);
        setError(null);
      } catch (err) {
        console.error(`Error fetching ${staffType.toLowerCase()} reports:`, err);
    ...
    fetchReportData();
  }, [staffType]); // staffType added to dependency array as it's used in fetchReportData

  return (
    <div className={styles.container}>
      <Card title={`${staffType} Activity Report`}>
        ...
        {reportData && (
          <div className={styles.reportContent}>
            <div className={styles.reportSection}>
              <h3 className={styles.sectionTitle}>Total {staffType} Staff</h3>
              <p className={styles.statValue}>{reportData.totalHousekeepingStaff}</p>
            </div>
                ...
                <ul className={styles.statList}>
                  {Object.entries(reportData.areaAssignments).map(([area, count]) => (
                    <li key={area} className={styles.statListItem}>
                      <span className={styles.statLabel}>{area}:</span> {count} staff assigned
                    </li>
                  ))}
                </ul>
              </div>
        ...
        {!loading && !error && !reportData && (
            <p>No {staffType.toLowerCase()} activity data available.</p>
        )}
      </Card>
    </div>
  );
