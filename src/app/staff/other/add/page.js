...
export default function AddOtherStaffPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/staff', { // Using the generic /api/staff endpoint
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(responseData.message || `Failed to add ${formData.employee_type || 'staff'}`);
      }
      
      setSubmitMessage(`${formData.employee_type || 'Staff'} added successfully! Employee ID: ${responseData.employeeId}`);
  ...
  return (
    <div className={styles.formContainer}>
      <Card title="Add Other Staff Member">
