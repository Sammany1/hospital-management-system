// filepath: /src/app/api/staff/route.js
...
export async function GET(request) {
  ...
  try {
    ...
    if (role === 'doctor') {
      ...
      joins += ` LEFT JOIN housekeeping_staff hk ON e.employee_id = hk.HK_employee_id
    ...
    console.error('API /api/staff GET Error:', error);
    if (error.code === 'ER_NO_SUCH_TABLE' || error.message.includes('Unknown column')) {
        return NextResponse.json({ 
            message: 'Error fetching staff data. Schema mismatch. Check server logs and doc.md.', 
            error: error.message 
        }, { status: 500 });
    }
    return NextResponse.json({ message: 'Error fetching staff data', error: error.message }, { status: 500 });
  } finally {
...
export async function POST(request) {
  ...
  try {
    ...
    if (employee_type === 'Doctor' && specialization) {
      ...
      await connection.execute('INSERT INTO housekeeping_staff (HK_employee_id) VALUES (?)', [employeeId]);
    ...
    return NextResponse.json({ 
      ...
      staff_id: employeeId // ensure staff_id is returned for consistency with GET
    }, { status: 201 });

  } catch (error) {
    if (connection) await connection.rollback();
    console.error('API /api/staff POST Error:', error);
    ...
    if (error.code === 'ER_DUP_ENTRY' || (error.message && error.message.toLowerCase().includes('duplicate entry'))) {
        if (error.message.includes('email_UNIQUE')) { // Check if your unique constraint on email is named 'email_UNIQUE'
            return NextResponse.json({ message: 'Error adding staff: Email already exists.', error: error.message }, { status: 409 }); // 409 Conflict
        }
        return NextResponse.json({ message: 'Error adding staff: Duplicate entry.', error: error.message }, { status: 409 });
    }
    return NextResponse.json({ message: 'Error adding staff', error: error.message }, { status: 500 });
  } finally {
    if (connection) connection.release();
    dbSemaphore.release();
  }
