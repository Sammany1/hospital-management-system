// filepath: /Users/alsammany/Documents/Personal/SPRING 25/Database/Project/GUI/hospital/src/app/api/staff/add-housekeeping/route.js
...
export async function POST(request) {
  ...
  try {
    ...
    // 3. Insert into housekeeping_staff table
    const housekeepingInsertQuery = `INSERT INTO housekeeping_staff (HK_employee_id) VALUES (?)`;
    ...
    return NextResponse.json({
      message: 'Housekeeping staff added successfully',
    ...
    console.error('Error adding housekeeping staff:', error);
    if (error.code === 'ER_DUP_ENTRY') {
        return NextResponse.json({ message: 'Failed to add housekeeping staff. Possible duplicate entry (e.g., email).', error: error.message }, { status: 409 });
    }
    ...
    return NextResponse.json({ message: 'Failed to add housekeeping staff', error: error.message, code: error.code, sqlMessage: error.sqlMessage }, { status: 500 });
  } finally {
