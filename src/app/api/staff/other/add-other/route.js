// /Users/alsammany/Documents/Personal/SPRING 25/Database/Project/GUI/hospital/src/app/api/staff/other/add-other/route.js
...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({ 
        message: `Staff member (${employeeType}) added successfully`, 
    ...
    if (error.code === 'ER_DUP_ENTRY') {
        return NextResponse.json({ message: `Failed to add staff member (${data.employeeType || 'Unknown Type'}). Possible duplicate entry (e.g., email).`, error: error.message }, { status: 409 });
    }
    ...
    return NextResponse.json({ message: `Failed to add staff member (${data.employeeType || 'Unknown Type'})`, error: error.message, code: error.code, sqlMessage: error.sqlMessage }, { status: 500 });
  } finally {
